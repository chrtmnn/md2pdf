...
3. On npmjs.com, add a Trusted Publisher for `chrtmnn/md2pdf` with workflow `release.yml` and environment `npm`, and under *Allowed actions* tick **`npm publish`**. A configuration created after 2026-09-03 allows only `npm stage publish` by default, and the workflow's direct `npm publish` is then refused with `E403 … OIDC permission denied for this action`: the identity matched, only the action was not allowed — the `v0.1.1` release ran into exactly that. npm does not validate the configuration when it is saved, and every field is case-sensitive.
...
- With `--toc` (`toc === 'always'`), every genuine doctoc block is removed from each document (`removeDoctocBlocks` in `doctoc-markers.ts`; documented examples stay, a lone END line is dropped, a START without END fails the merge), and `prependTocMarkers` in `merge-assembly.ts` puts an empty marker pair plus a `.document-break` in front of the first document, after its frontmatter. `runDoctoc` then sees a `pair` and fills it in place, so the relocation before the first `##` never applies and the merged PDF has exactly one TOC. START and END must be on separate lines: doctoc 2.5 does not recognise a pair on one line and prepends a second TOC. The empty pair also means doctoc writes no "Table of Contents" caption, unlike a freshly created block. The removed blocks are counted and reported in one warning.
